...
function QuizLinkPopup({ quizUrl, onClose }: { quizUrl: string; onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        <button
          className="absolute top-4 right-4 flex h-9 w-9 items-center justify-center rounded-full border-none text-2xl leading-none font-light text-white transition-all duration-200"
          style={{
        ...
        <div className="m-[3px] rounded-[17px] bg-white p-10 text-center">
          ...
          <h2 className="m-0 mb-3 text-[28px] leading-tight font-bold tracking-[-0.5px] text-gray-900">
            Ready to Begin?
          </h2>

          <p className="m-0 mb-8 text-base leading-relaxed text-gray-600">
            Click the link below to open the quiz in a new tab. The proctor will remain here to
...
export function RpcHandlers() {
  ...
  useEffect(() => {
    ...
    const handleShowQuizLink = async (): Promise<string> => {
      try {
    ...
    room.registerRpcMethod('frontend.showQuizLink', async () => await handleShowQuizLink());
  ...
  return <QuizLinkPopup quizUrl={quizPopup.quizUrl} onClose={() => setQuizPopup(null)} />;
}
